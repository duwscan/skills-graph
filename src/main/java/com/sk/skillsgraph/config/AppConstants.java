...
public final class AppConstants {
    ...
    public static final String EMBEDDING_MODEL = envString("EMBEDDING_MODEL", "text-embedding-3-large");
    /** Redis TTL for embedding cache entries (30 days). */
    public static final long EMBEDDING_CACHE_TTL_SECONDS = envLong("EMBEDDING_CACHE_TTL", 2_592_000L);
    ...
    public static final double SIMILARITY_DUPLICATE_THRESHOLD = envDouble("SIMILARITY_DUPLICATE_THRESHOLD", 0.90);
    /** Similarity threshold for review queue classification. */
    public static final double SIMILARITY_REVIEW_THRESHOLD = envDouble("SIMILARITY_REVIEW_THRESHOLD", 0.70);
    ...
    public static final int RAG_CANDIDATE_LIMIT = envInt("RAG_CANDIDATE_LIMIT", 100);
    ...
    public static final long EXTRACTION_CACHE_TTL_SECONDS = envLong("EXTRACTION_CACHE_TTL", 604_800L);
    ...
    public static final int LLM_CONCURRENCY_LIMIT = envInt("LLM_CONCURRENCY_LIMIT", 50);
    ...
    public static final int CO_OCCURRENCE_EDGE_THRESHOLD = envInt("CO_OCCURRENCE_EDGE_THRESHOLD", 20);
    /** Normalization factor for empirical edge weight increments. */
    public static final int CO_OCCURRENCE_NORMALIZATION_FACTOR = envInt("CO_OCCURRENCE_NORMALIZATION_FACTOR", 100);
    ...
    public static final int DISCOVERY_SIGNAL_THRESHOLD = envInt("DISCOVERY_SIGNAL_THRESHOLD", 5);
    /** Redis TTL for discovery signal counters (30 days). */
    public static final long DISCOVERY_SIGNAL_TTL_SECONDS = envLong("DISCOVERY_SIGNAL_TTL", 2_592_000L);
    ...
    public static final int RATE_LIMIT_READ = envInt("RATE_LIMIT_READ", 100);
    /** Extraction endpoint rate limit (requests/minute). */
    public static final int RATE_LIMIT_EXTRACT = envInt("RATE_LIMIT_EXTRACT", 20);
    /** Mutation endpoint rate limit (requests/minute). */
    public static final int RATE_LIMIT_MUTATE = envInt("RATE_LIMIT_MUTATE", 50);

    /** Extraction worker concurrency. */
    public static final int EXTRACTION_WORKER_CONCURRENCY = envInt("EXTRACTION_WORKER_CONCURRENCY", 10);
    ...
    public static final long BATCH_RESULT_TTL_SECONDS = envLong("BATCH_RESULT_TTL", 86_400L);
    /** Debounce interval for Typesense sync events. */
    public static final long TYPESENSE_SYNC_DEBOUNCE_MS = envLong("TYPESENSE_SYNC_DEBOUNCE_MS", 500L);
    ...
    /** Effective DB pool size resolved from env/profile defaults. */
    public static final int DB_POOL_SIZE = resolveDbPoolSize();
    ...
    private static int resolveDbPoolSize() {
        String override = System.getenv("DB_POOL_SIZE");
        if (override != null && !override.isBlank()) {
            return parseInt("DB_POOL_SIZE", override, DB_POOL_SIZE_DEV);
        }

        String profile = System.getenv("SPRING_PROFILES_ACTIVE");
        if (profile == null) {
            return DB_POOL_SIZE_DEV;
        }
        String normalized = profile.toLowerCase(Locale.ROOT);
        return normalized.contains("prod") ? DB_POOL_SIZE_PROD : DB_POOL_SIZE_DEV;
    }

    private static int envInt(String key, int fallback) {
        String value = System.getenv(key);
        return value == null || value.isBlank() ? fallback : parseInt(key, value, fallback);
    }
