...
When a skill becomes obsolete (e.g., "Adobe Flash"), it is deprecated with a pointer to its successor(s). Aliases are remapped, downstream consumers receive deprecation events, and the skill remains queryable (for historical data) but excluded from extraction and search.
...
When two skills are discovered to be duplicates (e.g., "Machine Learning" and "ML"), they are merged. One is chosen as the survivor, the other as the source. All aliases, edges, and references are transferred to the survivor.

### Tasks

| # | Task | Detail | Files |
|---|---|---|---|
| 6.2.1 | `SkillService.merge(sourceId, targetId)` | Full transactional merge: (1) Validate both skills. (2) Move all aliases from source to survivor. (3) Re-point all edges where source is the `source_skill_id` to survivor (skip if equivalent edge exists). (4) Re-point all edges where source is the `target_skill_id` to survivor (skip if equivalent edge exists). (5) Delete orphaned duplicate edges. (6) Set source `status = 'merged'`. (7) Create `superseded_by` edge from source to survivor. (8) Record changelog with full diff. (9) Fire PG NOTIFY. (10) Update Typesense: remove source, re-index survivor with merged aliases | `src/services/lifecycle/merge.ts` |
...
| 6.3.3 | Snapshot export | `bun run snapshot:create` — export entire taxonomy as JSON: `{ version, timestamp, skills: [...], relationships: [...], aliases: [...], locale_config: [...] }`. Write to `snapshots/skills-graph-v{version}-{date}.json` | `src/scripts/snapshot-create.ts` |
| 6.3.4 | Snapshot import | `bun run snapshot:import <file>` — import a snapshot to restore taxonomy state. Truncate all tables, insert snapshot data, rebuild Typesense index, regenerate embeddings | `src/scripts/snapshot-import.ts` |
...
- [ ] Snapshot file contains all skills, relationships, aliases, locale_config
...
- [ ] `bun run snapshot:create` exports valid snapshot
