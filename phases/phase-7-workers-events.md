...
| # | Task | Detail | Files |
|---|---|---|---|
| 7.8.1 | Worker entry point | `src/main/java/com/skillsgraph/worker/WorkerConfiguration.java` — starts all workers: extraction, discovery, full-text-search-sync, co-occurrence, reanalysis. Each worker runs in the same Bun process with independent event loops. Script: `./mvnw spring-boot:run -Dspring-boot.run.arguments=--workers` | `src/main/java/com/skillsgraph/worker/WorkerConfiguration.java` |
