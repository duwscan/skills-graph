...
| 4.1.2 | Token estimator | `estimateTokens(text: string): number` — approximate token count using word-count heuristic (words * 1.3) or `word-count heuristic / tiktoken-java` for accuracy. Used to determine chunk boundaries | `src/main/java/com/skillsgraph/service/extraction/TokenEstimator.java` |
