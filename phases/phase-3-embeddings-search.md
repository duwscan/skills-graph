...
| 3.2.2 | `findSimilarSkills(embedding, k, filters?)` | Return top-K nearest skills from the `skill_embeddings` PostgreSQL table. SQL: `SELECT se.skill_id, 1 - (se.embedding <=> $1::vector) AS similarity FROM skill_embeddings se ORDER BY se.embedding <=> $1::vector LIMIT $2`. Active skill IDs are maintained in a Redis set (synced from Neo4j) or a denormalized `status` column. Graph context is fetched from Neo4j via `Neo4jTemplate` after finding the IDs | `src/main/java/com/skillsgraph/service/VectorSearchService.java` |
